

dtr <- function (x, y){
  x = c(0,x)
  y = c(0,y)
  return (max(x-y) - min(x-y))
}
deu <- function (x, y) return (sqrt(sum((x-y)^2)))

tropicalLine <-  function (x,y, tk) {
    if (identical(x,y)) {
      mu =  (x)

    }
    if ((x[1] == y[1]) | (x[2] == y[2])) {
      mu = ((1-tk)*x+y*tk)

    }
    
    if ((x[1] == x[2]) | (y[1] == y[2])){
      mu = ((1-tk)*x+y*tk)

    }
    
    if (x[1]<y[1]) {
      a = x
      b = y
    }
    if (x[1]>y[1]) {
      a = y
      b = x
    }
    if ((a[2]<b[2]) & ((a[1]-a[2])<=(b[1]-b[2]))){
      t = (b[2]-a[2])/(b[1]-a[1])
      if (tk <= t) {
        mu = c(a[1]+tk*(b[1]-a[1]), a[2]+tk*(b[1]-a[1]))
      }
      if (t < tk) {
        mu = c(a[1]+tk*(b[1]-a[1]), b[2])
        
      }
    }
    
    if ((a[2]<b[2]) & ((a[1]-a[2])>(b[1]-b[2]))){
      t = (b[1]-a[1])/(b[2]-a[2])
      if (t >= tk) {
        mu = c(a[1]+tk*(b[2]-a[2]), a[2]+tk*(b[2]-a[2]))
      }
      if (t < tk) {
        mu = c(b[1],a[2]+tk*(b[2]-a[2]))
        
      }
    }
    
    if (a[2]>b[2]){
      t = (a[2]-b[2])/(a[2]-b[2]+b[1]-a[1])
      if (t >= tk) {
        mu = c(a[1],a[2]-tk*(a[2]-b[2]+b[1]-a[1]))
      }
      if (t < tk) {
        mu = c(a[1]+b[2]-a[2]+tk*(a[2]-b[2]+b[1]-a[1]),b[2])
      }
    }
    return (mu)
  }

distances <- function (a,b,c,x1,y1,c1){
  t <- seq(0,1, by= 0.01)
  distan = matrix(NA, nrow = length(t),ncol = 2)
  if (x1[1]>=y1[1]) {
    a1 = y1
    b1 = x1
  }
  else {
    a1=x1
    b1=y1
  }
  #muE = matrix(NA, nrow = length(t),ncol = 2)
  for (i in 1:length(t)){
    tk = t[i]
    muTR = tropicalLine (a,b,tk)
    muE= (1-tk)*a1+b1*tk
    distan[i,] =  c(deu(muE, c1), dtr(muTR, c))
  }
  distan = round(distan , 7)
  if ((distan[1,1] ==distan[length(t),2]) & (distan[1,2] ==distan[length(t),1])){
    distan[,1] = rev(distan[,1])
  }
  return (distan)
}

findEuclidean <- function (P){
  a = P[,1]
  b = P[,2]
  c = P[,3]
  dab = dtr(a,b)
  dbc = dtr(b,c)
  dac = dtr(c,a)
  a1 = c(0,0)
  b1 = c(dab,0)
  xc = (dac^2-dbc^2+dab^2)/(2*dab)
  yc = sqrt(dac^2-xc^2)
  c1 = c(xc, yc)
  P1 <- matrix(c(a1,b1, c1),nrow=length(a1))
  return (P1)
}

curvature <- function (P){
  P1 <- findEuclidean(P)
  curvS = 0
  curvF = 0
  for (j in 1:ncol(P)){
    a = P[,j%%ncol(P)+1]
    b = P[,(j+1)%%ncol(P)+1]
    c = P[,(j+2)%%ncol(P)+1]
    a1 = P1[,j%%ncol(P1)+1]
    b1 = P1[,(j+1)%%ncol(P1)+1]
    c1 = P1[,(j+2)%%ncol(P1)+1]
    distan = round(distances (a,b,c,a1,b1,c1), 5)
    if (all(distan[,1] <= distan[,2])) curvF = curvF +1 
    else if (all(distan[,1] >= distan[,2])) curvS = curvS +1
    else (return (0))
  }
  if (curvS == 3) return (-1)
  if (curvF == 3) return (1)
  else return (0)
}

# skinny triangle
a = c(1,3)
b= c(0,0)
c= c(3,2)


# fat triangle
a = c(0,2)
b = c(1,0)
c = c(3,3)
#a1 = c(3/2,2.598076)
#b1 = c(0,0)
#c1 = c(3,0)

#undefined on the paper
a = c(0,2)
b = c(2,0)
c = c(3,4)
#a1= c(2, sqrt(5))
#b1 = c(0,0)
#c1 = c(4, 0)

#fat
a = c(0,4)
b = c(3,0)
c = c(5, 6)
#a1 = c(5, 2*sqrt(6) )
#b1 = c(0,0)
#c1= c(6,0)

#skinny 

#a1 = c(9/4,sqrt(16-(9/4)^2))
#b1 = c(0,0)
#c1= c(6,0)
a = c(0,0)
b = c(2,4)
c = c(5,1)

#undefined
a = c(0,0)
b = c(448,449)
c = c(452,256)

#P is the tropical triangle
P<- matrix(c(a,b, c),nrow=length(a))
#P1 is the corresponding euclidean

curvature(P)

#P1 <- matrix(c(a1,b1, c1),nrow=length(a1))


P1 <- findEuclidean(P)
a1 = P1[,1]
b1= P1[, 2]
c1 = P1[, 3]
distan = distances (a,b,c,P1[,1],P1[, 2],P1[, 3])
t <- seq(0,1, by= 0.01)
plot(t, distan[,2], type = 'l', ylim= c(1, 4))
lines(t, distan[,1], type = 'l')
distan = distances (a,c,b,a1,c1,b1)
t <- seq(0,1, by= 0.01)
plot(t, distan[,2], type = 'l')
lines(t, distan[,1], type = 'l')
distan = distances (c,b,a,c1,b1,a1)
t <- seq(0,1, by= 0.01)
plot(t, distan[,2], type = 'l')
lines(t, distan[,1], type = 'l')
all(distan[,1] <= distan[,2])
